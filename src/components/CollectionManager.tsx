...
export default function CollectionManager({
  ...
  const handleColContextMenu = (e: React.MouseEvent, colId: string) => {
    e.preventDefault();
    e.stopPropagation();
    // For collections in the sidebar, align to where the click happened but constrained to the item
    setColContextMenu({ 
      x: e.clientX, 
      y: e.clientY, 
      colId 
        ...
        <div key={col.id} className="select-none">
          ...
          >
            <div className="flex items-center gap-2 flex-grow min-w-0">
              ...
              <span className={`text-xs truncate ${isSelected ? 'font-bold' : ''}`}>{col.name}</span>
            </div>
  ...
  const filteredItems = getItemsForCollection(selectedCollectionId);
      ...
      <div className="w-full md:w-64 border-r border-[#E6E0D5] dark:border-[#2D3137] flex flex-col bg-[#FAF8F5]/30 dark:bg-zinc-950/20">
        ...
        <div className="flex-grow overflow-y-auto py-2">
          ...
          >
            ...
            <span className="ml-auto text-[9px] font-mono opacity-50">{mediaItems.length}</span>
          </div>
              ...
              <div className="flex-grow overflow-y-auto p-6 md:p-8">
                {filteredItems.length > 0 ? (
                      ...
                      >
                        ...
                        <div className="p-3 space-y-2">
                          <div className="flex items-center justify-between gap-2">
                            <div className="text-xs font-bold text-zinc-800 dark:text-zinc-100 truncate flex-grow">{item.title}</div>
                            {!isMasterSelected && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center opacity-40 py-20">
                    ...
                    <p className="text-xs font-bold uppercase tracking-[0.2em]">此归档尚无项目</p>
                  </div>
                )}
              </div>
            </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="border-b border-zinc-100 dark:border-zinc-800 pb-2">
                <h3 className="text-sm font-bold uppercase tracking-wider text-[#2B1E19] dark:text-zinc-100 font-mono flex items-center gap-2">
                  <FolderPlus size={16} />
        ...
        {contextMenu && (
          ...
          >
            <motion.div
              ...
              className="absolute bg-white dark:bg-[#1c1e22] border border-[#E6E0D5] dark:border-zinc-800 shadow-2xl py-1.5 min-w-[220px] max-h-[400px] overflow-y-auto custom-scrollbar"
              style={{ 
                      ...
                      <React.Fragment key={col.id}>
                        <button
                          onClick={() => {
                            ...
                            onUpdateItemCollections(targetItem.id, newCollections);
                            setContextMenu(null);
                          }}
        ...
        {colContextMenu && (
          ...
          >
            <motion.div
              ...
              transition={{ duration: 0.1 }}
              className="absolute w-44 bg-white dark:bg-[#1c1e22] border border-[#E6E0D5] dark:border-zinc-800 shadow-2xl py-1.5"
              style={{ 
            ...
            >
              <button
                onClick={() => {
                  ...
                  if (col) {
                    const newName = window.prompt('修改合集名称:', col.name);
                    if (newName && newName.trim()) {
                      onUpdateCollection(col.id, { name: newName.trim() });
                    }
                  }
                  setColContextMenu(null);
                }}
