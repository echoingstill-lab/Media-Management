        ...
        >
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-none border ${
              darkMode 
                ? 'bg-[#111214] border-[#2d3137] text-zinc-400' 
                : 'bg-[#FAF8F5] border-[#E6E0D5] text-[#4A3B32]'
            }`}>
              <Bookmark size={16} />
            </div>
            <div>
              <h2 className="font-bold tracking-tight uppercase text-zinc-800 dark:text-zinc-100 flex flex-wrap items-baseline gap-3">
                <span className="font-serif text-3xl font-bold text-[#4A3B32] dark:text-[#DDDAC4] leading-none">
                  {formatChineseMonth(monthStr)}
                </span>
                
                {labelType === 'current' && (
                  <span className="font-sans font-bold text-xs px-2.5 py-0.5 border border-amber-500/30 bg-amber-500/5 text-amber-700 dark:text-amber-400 rounded-none">
                    本月清单
                  </span>
                )}
                {labelType === 'next' && (
                  <span className="font-sans font-bold text-xs px-2.5 py-0.5 border border-[#E6E0D5] dark:border-zinc-850 bg-zinc-55/10 text-zinc-650 dark:text-zinc-300 rounded-none">
                    下月计划
                  </span>
                )}
                {labelType === 'past' && (
                  <span className="font-sans font-bold text-xs px-2.5 py-0.5 border border-zinc-300 dark:border-zinc-800 bg-zinc-100/10 dark:bg-zinc-850/10 text-zinc-400 dark:text-zinc-500 rounded-none">
                    往期存档
                  </span>
                )}
                  ...
                  >
                      ...
                      <div className="p-3.5 space-y-2">
                        {hasItems ? (
                                ...
                                <div className="flex items-center gap-2.5 min-w-0 flex-grow">
                                  {item.status === 'progress' ? (
                                    <div className="relative group/tooltip shrink-0">
                                      ...
                                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/tooltip:block bg-zinc-950 dark:bg-zinc-800 text-white dark:text-zinc-200 text-[10px] font-sans font-semibold py-1 px-2 whitespace-nowrap shadow-lg z-50 pointer-events-none rounded-none border border-zinc-800 dark:border-zinc-700">
                                        进行中... 点击标记已完成
                                      </div>
                                    </div>
                                  ) : (
                          ...
                          <div className="text-center py-10 border border-dashed border-[#E6E0D5]/60 dark:border-zinc-800/80 bg-[#FAF8F5]/30 dark:bg-zinc-900/10 text-zinc-400 dark:text-zinc-500 rounded-none">
                            <span className="text-xs font-mono uppercase tracking-wider block font-bold">暂无规划</span>
                            <span className="text-[10px] text-zinc-400 dark:text-zinc-500 block mt-1">点击下方录入清单目标</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    {addingToTarget?.type === type && addingToTarget?.month === monthStr && (
                      <div className="absolute inset-x-0 bottom-0 top-[45px] z-20 bg-[#FCFAF7] dark:bg-[#191b1e] border-t border-[#4A3B32] dark:border-[#DDDAC4] shadow-xl animate-in slide-in-from-bottom-2 duration-200 flex flex-col overflow-hidden">
                        <div className="p-3 border-b border-[#E6E0D5] dark:border-zinc-800 flex items-center justify-between bg-white dark:bg-[#111214]">
                          <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-800 dark:text-zinc-200 font-mono">
                        ...
                        <div className="p-4 space-y-4 overflow-y-auto flex-grow bg-white/50 dark:bg-transparent">
                          ...
                          <form onSubmit={handlePopupQuickAdd} className="space-y-2">
                            <div className="flex gap-2">
                              ...
                              />
                              <button type="submit" className="px-3 py-2 bg-[#4A3B32] dark:bg-[#DDDAC4] text-white dark:text-[#111214] font-mono font-bold text-[10px] uppercase cursor-pointer">
                                + 新增 / NEW
                              </button>
                            </div>
                          </form>
                          ...
                          <div className="space-y-3">
                            <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-400 font-mono block">从库中选择</span>
                    ...
                    <div className="p-3.5 border-t border-[#E6E0D5]/60 dark:border-zinc-800/80 bg-zinc-50/40 dark:bg-zinc-900/10 text-center">
                      <button
                        ...
                        className="text-xs font-mono font-bold tracking-widest text-[#756256] hover:text-[#4A3B32] dark:text-zinc-400 dark:hover:text-white uppercase inline-flex items-center gap-1 transition-colors cursor-pointer"
                      >
                        <Plus size={11} />
